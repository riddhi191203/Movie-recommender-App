...
def movie_poster_fetcher(imdb_link):
    try:
        ...
        raw_data = urlopen(movie_poster_link).read()
        ...
        st.warning("Unable to load poster")


def get_movie_info(imdb_link):
    try:
        url_data = requests.get(imdb_link, headers=hdr, timeout=10).text
        s_data = BeautifulSoup(url_data, 'html.parser')
        ...
        descr = imdb_content["content"]
        parts = [p.strip() for p in descr.split('.') if p.strip()]

        # Safe extraction
        director = parts[0] if len(parts) > 0 else "Director: N/A"
        cast = "Cast: " + parts[1] if len(parts) > 1 else "Cast: N/A"
        story = "Story: " + parts[2] + "." if len(parts) > 2 else "Story: N/A"

        rating_tag = s_data.find("span", class_="sc-bde20123-1 iZlgcd")
        rating = rating_tag.text if rating_tag else "N/A"
        total_rating = "Total Rating count: " + rating

        return director, cast, story, total_rating

    except Exception as e:
        print("Movie info error:", e)
        return "Director: N/A", "Cast: N/A", "Story: N/A", "Rating: N/A"
